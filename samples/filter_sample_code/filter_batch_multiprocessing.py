...
if __name__ == '__main__':
    ...
    # Previous approach for multiprocessing which failed for shared global variable
    """ max_cores = multiprocessing.cpu_count()
        with Pool(max_cores) as p:
            p.map(main, args=(files, final_result)) """
    ...
    with Manager() as manager:
        ...
        processes = []
        for f in files:
            p = Process(target=main, args=(f, final_result))
            p.start()
            processes.append(p)
        for p in processes:
            p.join()
